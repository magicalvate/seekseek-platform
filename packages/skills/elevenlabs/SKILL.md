...
- **Output files** are saved relative to the current working directory.
- **Long text** is handled automatically by the API. For very long documents (>5000 chars), consider using `batch` with `--split-paragraphs`.
- **Rate limits** apply per your ElevenLabs plan.
- **Character usage** counts against your ElevenLabs monthly quota.
